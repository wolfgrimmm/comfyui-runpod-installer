...
def viewer_status():
    """Check ComfyViewer installation and running status"""
    try:
        import socket

        # Check if installed
        installed = os.path.exists('/app/comfyviewer/.next/BUILD_ID')

        # Check if running (port 3001)
        running = False
        pid = None
        if os.path.exists('/tmp/comfyviewer.pid'):
            try:
                with open('/tmp/comfyviewer.pid', 'r') as f:
        ...
        return jsonify({
            ...
            'pid': pid,
            'port': 3001,
            'url': 'http://localhost:3001' if running else None
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/viewer/install', methods=['POST'])
def install_viewer():
    """Install ComfyViewer"""
    try:
        # Check if already installed
        if os.path.exists('/app/comfyviewer/.next/BUILD_ID'):
            return jsonify({'success': True, 'message': 'ComfyViewer already installed'})

        # Run installation script
        result = subprocess.run(
            ['/app/scripts/install_comfyviewer.sh'],
            capture_output=True,
            text=True,
        ...
        if result.returncode == 0:
            return jsonify({
                'success': True,
                'message': 'ComfyViewer installed successfully'
            })
        else:
            return jsonify({
                'success': False,
        ...
        return jsonify({
            'success': False,
            'message': 'Installation timeout (10 minutes)'
        }), 500
    except Exception as e:
        return jsonify({
            'success': False,
...
def start_viewer():
    """Start ComfyViewer service"""
    try:
        # Check if installed
        if not os.path.exists('/app/comfyviewer/.next/BUILD_ID'):
            return jsonify({
                'success': False,
                'message': 'ComfyViewer not installed. Please install first.'
            }), 400

        # Set current user for viewer
        env = os.environ.copy()
        env['COMFYUI_USER'] = manager.current_user or 'default'
        env['COMFYVIEWER_PORT'] = '3001'

        # Start the viewer
        result = subprocess.run(
            ['/app/start_comfyviewer.sh'],
            capture_output=True,
            text=True,
            env=env,
            timeout=30
        ...
        if result.returncode == 0:
            return jsonify({
                'success': True,
                'message': 'ComfyViewer started successfully',
                'port': 3001
            })
        else:
            return jsonify({
                'success': False,
                'message': 'Failed to start ComfyViewer',
                'error': result.stderr[-500:] if result.stderr else result.stdout[-500:]
            }), 500
    except Exception as e:
        return jsonify({
            'success': False,
            'message': f'Error starting viewer: {str(e)}'
        }), 500

@app.route('/api/viewer/stop', methods=['POST'])
def stop_viewer():
    """Stop ComfyViewer service"""
    try:
        result = subprocess.run(
            ['/app/stop_comfyviewer.sh'],
            capture_output=True,
            text=True,
            timeout=10
        )
        ...
        return jsonify({
            'success': True,
            'message': 'ComfyViewer stopped'
        })
    except Exception as e:
        return jsonify({
            'success': False,
            'message': f'Error stopping viewer: {str(e)}'
        }), 500

@app.route('/api/viewer/logs')
def viewer_logs():
    """Get ComfyViewer logs"""
    try:
        log_file = '/workspace/comfyviewer.log'
